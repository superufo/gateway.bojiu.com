...
import (
	...
	"errors"
	"fmt"
	"gateway.bojiu.com/internal/net/gstream/pb"
...
func NewGrpcClientConn(url string) (*GrpcClientConn, error) {
	...
	gcn := &GrpcClientConn{
		GrpcRecvData:    make(chan *pb.StreamResponseData, 5),
		GrpcSendData:    make(chan *pb.StreamRequestData, 5),
	...
	log.ZapLog.With(zap.Any("conn.GetState", conn.GetState())).Info("Run")
	return gcn, nil
}
...
func (gc *GrpcClientConn) sendStream() {
	...
	for {
		...
		case sd = <-gc.GrpcSendData:
			...
			log.ZapLog.With(zap.Any("msg", sd.GetMsg()), zap.Any("data", sd.GetData()),
...
func (gc *GrpcClientConn) FowardToBackServer(clientId string, msg uint32, secret []byte, serialNum []byte, data []byte) (err error) {
...
func (gc *GrpcClientConn) Close() {
	if !gc.isDisConnection {
		gc.lock.Lock()
		gc.isDisConnection = true
		gc.clientStream.CloseSend()
		gc.clientConn.Close()
